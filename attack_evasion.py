...
# Attack parameters
batch_size     = 9 # Other images of the target - we know nine always exist
...
trainer      = optimizer.minimize(-target_loss, var_list=[delta]) # Maximize instead of minimize
end_vars     = tf.global_variables()
new_vars     = [x for x in end_vars if x.name not in start_vars]
init         = tf.variables_initializer(var_list=[delta]+new_vars)

# Create result directory
meta_folder    = 'paper_attacks_evasion_%s' % (mask_name)
...
for target_person in target_person_list:
    ...
    # Pick a number of evading targets
    evade_idx       = np.random.choice(np.setdiff1d(np.arange(num_samples_target), anchor_idx), size=batch_size,
                                       replace=False)
    x_target_evade  = x_target_person[evade_idx]
    # Get their features
    x_target_real_features = model.predict(x_target_person)
    
    # Construct feed dictionary
    feed_dict = {x_input: np.arctanh((x_target_evade - 1/2) * 2 * 0.999999),
    ...
    winner_idx      = np.argmax(feature_loss_matrix, axis=0)
    # Instantly overwrite
