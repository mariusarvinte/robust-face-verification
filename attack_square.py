...
from aux_masks import gen_candidate_masks
...
candidate_masks = gen_candidate_masks()
...
x_target_features = tf.placeholder(dtype=np.float32, shape=(batch_size, 128))
# Tensor mask
loss_mask    = tf.placeholder(dtype=np.float32, shape=(batch_size, 64, 64, 3))
...
# Build another graph for attacking all candidate masks
delta_candidate = tf.Variable(tf.random.uniform(shape=(num_candidates, 64, 64, 3),
                                          minval=-0.5, maxval=0.5, dtype=tf.float32),
    dtype=np.float32)
# Input and target
x_input_cand  = tf.placeholder(dtype=np.float32, shape=(num_candidates, 64, 64, 3))
x_target_cand_features = tf.placeholder(dtype=np.float32, shape=(num_candidates, latent_dim))
# Mask tensor
cand_mask = tf.constant(candidate_masks, dtype=np.float32)
# Add adversarial noise
x_adv_cand = tf.tanh(x_input_cand + tf.multiply(cand_mask, delta_candidate))/2 + 1/2
# Get features
adv_cand_features = model(x_adv_cand)
# Feature loss
cand_feature_loss = tf.reduce_sum(tf.square(adv_cand_features - x_target_cand_features), axis=-1)
target_cand_loss = tf.reduce_sum(cand_feature_loss)

# Adam optimizer
start_vars        = set(x.name for x in tf.global_variables())
optimizer_cand    = tf.train.AdamOptimizer(learning_rate=1e-1)
trainer_cand      = optimizer_cand.minimize(target_cand_loss, var_list=[delta_candidate])
end_vars          = tf.global_variables()
new_vars          = [x for x in end_vars if x.name not in start_vars]
init_cand         = tf.variables_initializer(var_list=[delta_candidate]+new_vars)

# Create result directory
meta_folder    = 'paper_attacks_patch_search'
...
for target_person in target_person_list:
    ...
    x_others_random = np.arctanh((x_others_person[others_idx] - 1/2) * 2 * 0.999999)
    
    # These are the masks used for attacking
    best_mask_idx = np.zeros((len(x_others_random)), dtype=np.int)
    # For each adversary, perform a search for the best mask location
    for adversary_idx in tqdm(range(len(x_others_random))):
        # Wipe variables
        session.run(init)
        session.run(init_cand)
        
        # Construct feed dictionary
        feed_dict = {x_input_cand: np.repeat(x_others_random[adversary_idx][None, :], num_candidates, axis=0),
                     x_target_cand_features: np.repeat(x_target_real_features[anchor_idx][None, :], num_candidates, axis=0)}
        
        # Log
        cand_feature_loss_matrix = np.zeros((num_internal_iterations, num_candidates))
        # Run for a small(er) number of steps
        for step_idx in range(num_internal_iterations):
            _, cand_feature_loss_np = session.run([trainer_cand, cand_feature_loss],
                                                  feed_dict=feed_dict)
            # Log
            cand_feature_loss_matrix[step_idx] = cand_feature_loss_np
    
        # Find the best mask location for each attacker
        best_mask_idx[adversary_idx] = np.argmin(cand_feature_loss_np)
