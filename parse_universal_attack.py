...
mask_style = 'universal_patch' # Only valid option here
mask_size  = 10
...
for person_idx, target_person in enumerate(target_person_list):
    ...
    # Compute AUC ad-hoc
    min_dist = np.minimum(np.min(target_mirror_loss), np.min(other_mirror_loss))
    max_dist = np.maximum(np.max(target_mirror_loss), np.max(other_mirror_loss))
    thresholds = np.linspace(min_dist-1e-6, max_dist+1e-6, num_thresholds)
    # Manual loop
    local_int_fpr, local_int_tpr = np.zeros((num_thresholds,)), np.zeros((num_thresholds,))
    for idx, threshold in enumerate(thresholds):
        local_int_fpr[idx] = np.mean(target_mirror_loss > threshold)
        local_int_tpr[idx] = np.mean(other_mirror_loss > threshold)
    local_area_int = auc(local_int_fpr, local_int_tpr)
    ...
    try:
        ...
        continue
    # Unwrap
    adv_feature_loss = contents['best_feature_loss']
    local_adv  = contents['x_adv_np']
    best_delta = contents['best_delta']
    
    # Compute adversarial AUC ad-hoc
    min_dist = np.minimum(np.min(target_mirror_loss), np.min(adv_feature_loss))
    max_dist = np.maximum(np.max(target_mirror_loss), np.max(adv_feature_loss))
    thresholds = np.linspace(min_dist-1e-6, max_dist+1e-6, num_thresholds)
    # Manual loop
    local_fpr, local_tpr = np.zeros((num_thresholds,)), np.zeros((num_thresholds,))
    for idx, threshold in enumerate(thresholds):
        local_fpr[idx] = np.mean(target_mirror_loss > threshold)
        local_tpr[idx] = np.mean(adv_feature_loss > threshold)
    local_adv_area = auc(local_fpr, local_tpr)
...
universal_adv_aupr  = auc(adv_recall, adv_prec)
