...
from aux_mask_search import create_strided_masks, compute_gradient_magnitudes
...
C               = 10
candidate_masks = create_strided_masks(mask_size=20, stride=5)
...
image_generator = ImageDataGenerator(width_shift_range=0,
...
for step_idx in tqdm(range(num_steps)):
    ...
    # Get gradients
    grad_np = session.run(grad, feed_dict={x_adv: x_match_gen_half_batch,
                                           x_adv_pair: x_match_real_half_batch})
    # Search for best (worst) mask locations
    x_gen_mask_half_batch = compute_gradient_magnitudes(grad_np, x_match_gen_half_batch,
                                                        candidate_masks, model, x_match_real_half_batch,
                                                        False, C)
    # Perform a random erasure in the mask location
    x_match_gen_half_batch = x_match_gen_half_batch * (1 - x_gen_mask_half_batch) + \
    np.random.randint(low=0, high=256, size=x_gen_mask_half_batch.shape) / 256. * x_gen_mask_half_batch
...
#                diff_after = model.predict([x_orig, x_match_gen_half_batch[patch_attack_idx]])
    ...
    # Get gradients
    grad_np = session.run(grad, feed_dict={x_adv: x_fake_gen_half_batch,
                                           x_adv_pair: x_match_real_half_batch})
    # Search for best (worst) mask locations
    x_fake_mask_half_batch = compute_gradient_magnitudes(grad_np, x_fake_gen_half_batch,
                                                         candidate_masks, model, x_fake_gen_half_batch,
                                                         False, C)
    # Perform a random erasure in the mask location
    x_fake_gen_half_batch = x_fake_gen_half_batch * (1 - x_fake_mask_half_batch) + \
    np.random.randint(low=0, high=256, size=x_fake_mask_half_batch.shape) / 256. * x_fake_mask_half_batch
